...
<nav x-data="{ mobileOpen: false }"
        ...
        <div class="flex justify-between h-20">
            ...
            <div class="hidden md:flex items-center space-x-8 text-sm font-medium">
                ...
                <div class="relative group h-full flex items-center" x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
                    <button class="flex items-center gap-1 hover:text-hmj-purple dark:hover:text-purple-300 transition text-gray-700 dark:text-gray-300 focus:outline-none">
                        BIDANG <i class="fas fa-chevron-down text-[10px] transition-transform duration-300" :class="open ? 'rotate-180' : ''"></i>
                    </button>
    ...
    <div x-show="mobileOpen" x-transition
        ...
        <div x-data="{ expanded: false }">
            <button @click="expanded = !expanded" class="flex items-center justify-between w-full text-left text-gray-700 dark:text-gray-300 focus:outline-none">
                <span>BIDANG</span>
                <i class="fas fa-chevron-down text-xs transition-transform duration-300" :class="expanded ? 'rotate-180' : ''"></i>
            </button>

            <div x-show="expanded" x-collapse class="pl-4 mt-2 space-y-2 border-l-2 border-gray-100 dark:border-white/10 ml-1">
                @foreach($navDepartments as $dept)
                <a href="{{ route('department.show', $dept->slug) }}" wire:navigate
